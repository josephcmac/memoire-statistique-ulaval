...
create_graphics <- function(df, age, sex) {
  df |>
    ggplot() +
    geom_point(aes(arsenic, sigma)) +
    geom_smooth(aes(arsenic, sigma), method=sen, color="red", formula=y~x) +
    labs(
      title = expression("Régression robuste du logarithme du écart-type des MRC selon l'Arsenic " ~ PM[2.5]),
  ...
  lapply(names(l), function(l_name) {
    ggsave(
      filename = fs::path(here::here(
        "text", "figures", "arsenic_regression_sigma", l_name
      ), ext = "png"),
      plot = l[[l_name]]
    )
  })
